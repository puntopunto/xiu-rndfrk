...
#TODO: check args is accessible for installer during installing 'Rust'.
...
USER "$build_user"
RUN make online && make update && make build;
...
ENV PATH="$app_dir"
#TODO: проверить, нужны ли переменные APP и APP_CONFIG.
ENV APP="$app"
...
### Copy app
VOLUME '/app/config'
COPY    --from=builder --chown="$app_owner:$appgroup" --chmod=$app_dir_perms "$distribution/*" './'
            #"$distribution/$app", \
            #"$distribution/$web_server", \
            #"$distribution/$pprtmp_server" \
                #./

### Setup app - install sysconf, setup timezone, add user/group/perms, and remove unnecessary packs and delete cache
#TODO: check for smaller layers qty, if possible.
RUN $apkq upgrade --latest && \
    $apkq add --latest 'alpine-conf' && \
    setup-timezone -i "$TZ" && \
    addgroup -S "$appgroup" && \
    adduser -G "$appgroup" -g "$user_gecos" -s "$user_shell" -h "$user_home" -H -D -S "$app_user" && \
...
HEALTHCHECK --interval=5m --timeout=30s --start-period=5s --retries=3 \
        ...
        CMD [ '/bin/ping' '-q' '-c' "$hc_count" '-W' "$hc_wait" '-w' "$hc_timer" 'www.ru' && exit $? ]
